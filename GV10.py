...
class TestReport:
    ...
    def get_total_errors(self):
        ...
        if self.enabled_checks["special_chars"]:
            total += len([i for i in self.issues if i[0] == "Special Character Error"])
        if self.enabled_checks["spacing"]:
            total += len([i for i in self.issues if i[0] in ["Spacing Error", "Missing Space Error", "Extra Space Error"]])
        if self.enabled_checks["case_sensitivity"]:
            total += len([i for i in self.issues if i[0] == "Case Sensitivity Error"])
        if self.enabled_checks["comments"]:
            total += len([i for i in self.issues if i[0] == "Comment Error"])
        return total
    ...
    def generate_report(self, format="text"):
        if format == "text":
            ...
            if self.enabled_checks["special_chars"]:
                special_char_errors = [i for i in self.issues if i[0] == "Special Character Error"]
                content += (
                    f"Special Character Errors:\n" + (" - None found\n" if not special_char_errors else
                    "\n".join(f" - Line {i[2]}: {i[1]}" for i in special_char_errors)) + "\n\n"
                )

            if self.enabled_checks["spacing"]:
                spacing_errors = [i for i in self.issues if i[0] in ["Spacing Error", "Missing Space Error", "Extra Space Error"]]
                content += (
                    f"Spacing Issues:\n" + (" - None found\n" if not spacing_errors else
                    "\n".join(f" - Line {i[2]}: {i[1]}" for i in spacing_errors)) + "\n\n"
                )

            if self.enabled_checks["case_sensitivity"]:
                case_errors = [i for i in self.issues if i[0] == "Case Sensitivity Error"]
                content += (
                    f"Case Sensitivity Errors:\n" + (" - None found\n" if not case_errors else
                    "\n".join(f" - Line {i[2]}: {i[1]}" for i in case_errors)) + "\n\n"
                )

            if self.enabled_checks["comments"]:
                comment_errors = [i for i in self.issues if i[0] == "Comment Error"]
                content += (
                    f"Comment Issues:\n" + (" - None found\n" if not comment_errors else
                    "\n".join(f" - Line {i[2]}: {i[1]}" for i in comment_errors)) + "\n\n"
                )
            ...
            return {
                ...
                "special_char_errors": [{"line": i[2], "description": i[1]} for i in self.issues if i[0] == "Special Character Error"] if self.enabled_checks["special_chars"] else [],
                "spacing_errors": [{"line": i[2], "description": i[1]} for i in self.issues if i[0] in ["Spacing Error", "Missing Space Error", "Extra Space Error"]] if self.enabled_checks["spacing"] else [],
                "case_errors": [{"line": i[2], "description": i[1]} for i in self.issues if i[0] == "Case Sensitivity Error"] if self.enabled_checks["case_sensitivity"] else [],
                "comment_errors": [{"line": i[2], "description": i[1]} for i in self.issues if i[0] == "Comment Error"] if self.enabled_checks["comments"] else [],
                "stats": self.stats
            }
        else:
...
class GherkinChecker:
    ...
    def __init__(self, spellchecker=None, spell_cache=None, custom_words=None, spell_lock=None, feature_type="standard", spellcheck_enabled=True, spellcheck_language="en_UK"):
        ...
        self.keyword_pattern = re.compile(r"\b(?:Feature|Scenario|Given|When|Then|And|But|Background|Scenario Outline|Examples)\b")
        self.word_pattern = re.compile(r"\b\w+\b")
        self.placeholder_pattern = re.compile(r'<"[^">]*">|<"[^"]*$')  # Matches placeholders with optional missing >
        self.fixed_value_pattern = re.compile(r'"[^"]*"')
        self.bad_placeholder_pattern = re.compile(r"'[^']*'")
        self.special_char_pattern = re.compile(r'[!@#$%^&*()+={}\[\]|\\:;"\'<>,.?/~`]')
        self.double_space_pattern = re.compile(r'\s{2,}')
    ...
    def process_file(self, file, report):
        ...
        try:
            ...
            report.add_issue("Syntax Error", f"File not found: {file}", 1)
            return False
        except UnicodeDecodeError:
            logging.error(f"Unable to decode file: {file}")
            report.add_issue("Syntax Error", f"Unable to decode file: {file}", 1)
            return False
        ...
        then_count = 0
        and_count = 0
        placeholder_positions = {}  # Track placeholder order in steps
        ...
        for i, line in enumerate(lines, 1):
            ...
            if self.comment_pattern.match(line):
                if not line.startswith('#'):
                    report.add_issue("Comment Error", f"Unrecognized comment style: {line}", i)
                continue

            # Check case sensitivity (only first letter of keywords should be uppercase)
            words = line.split()
            for word in words:
                if self.keyword_pattern.match(word):
                    if not word[0].isupper() or any(c.isupper() for c in word[1:] if c.isalpha()):
                        report.add_issue("Case Sensitivity Error", f"Keyword '{word}' should have only first letter uppercase", i)
                elif word and not word in ["Feature:", "Scenario:", "Given:", "When:", "Then:", "And:", "But:", "Background:", "Scenario Outline:", "Examples:"] and any(c.isupper() for c in word[1:] if c.isalpha()):
                    report.add_issue("Case Sensitivity Error", f"Word '{word}' should be lowercase except for Gherkin keywords", i)
            ...
            if line.startswith("Feature"):
                ...
                if line != "Examples:":
                    report.add_issue("Syntax Error", f"Invalid Examples separator: {line}, should be 'Examples:'", i)
                in_examples = True
            elif in_examples and line.startswith('|'):
                if not examples_headers:
                    examples_headers = [h.strip() for h in line.split('|')[1:-1] if h.strip()]
                    # Validate special characters in headers (allow |, flag others)
                    for header in examples_headers:
                        special_chars = self.special_char_pattern.findall(header)
                        if special_chars and '|' not in special_chars:
                            report.add_issue("Special Character Error", f"Invalid special character in header '{header}': {''.join(special_chars)}", i)
                else:
                    total_iterations += 1
                ...
                if current_scenario and not in_examples:
                    ...
                    if self.double_space_pattern.search(line):
                        report.add_issue("Spacing Error", f"Double spaces detected in: {line}", i)

                    # Check for missing spaces (concatenated words)
                    words = line.split()
                    for j in range(len(words) - 1):
                        if not words[j + 1][0].isspace() and words[j][-1].isalnum() and words[j + 1][0].isalnum():
                            report.add_issue("Missing Space Error", f"Missing space between '{words[j]}' and '{words[j + 1]}'", i)

                    # Check for extra spaces after keywords
                    for keyword in ["Given", "When", "Then", "And", "But", "is"]:
                        if keyword in line:
                            index = line.index(keyword)
                            if index + len(keyword) < len(line) and line[index + len(keyword)].isspace():
                                next_char = line[index + len(keyword) + 1] if index + len(keyword) + 1 < len(line) else ''
                                if next_char and next_char.isspace():
                                    report.add_issue("Extra Space Error", f"Extra space after '{keyword}' in: {line}", i)

                    # Check placeholders and their order
                    placeholders = self.placeholder_pattern.findall(line)
                    for idx, ph in enumerate(placeholders):
                        ph_clean = ph.strip('<>').strip('"')
                        if not ph.endswith('>'):
                            report.add_issue("Invalid Placeholder Syntax", f"Missing closing '>' in placeholder '{ph}'", i)
                        # Track position for order check
                        placeholder_positions[ph_clean] = idx
                        # Check special characters in placeholders
                        special_chars = self.special_char_pattern.findall(ph_clean)
                        if special_chars and '|' not in special_chars:
                            report.add_issue("Special Character Error", f"Invalid special character in placeholder '{ph_clean}': {''.join(special_chars)}", i)
            ...
            # Check special characters (allow | in Examples, flag others)
            if not in_examples:
                special_chars = self.special_char_pattern.findall(line)
                if special_chars and '|' not in special_chars:
                    report.add_issue("Special Character Error", f"Invalid special character: {''.join(special_chars)}", i)
            ...
            # Check spaces inside double quotes
            quotes = self.fixed_value_pattern.findall(line)
            for quote in quotes:
                if quote.strip() != quote:
                    report.add_issue("Spacing Error", f"Leading/trailing spaces in quote '{quote}'", i)
                if self.double_space_pattern.search(quote):
                    report.add_issue("Spacing Error", f"Double spaces in quote '{quote}'", i)
        ...
        # Placeholder mismatch and order check with Examples
        if examples_headers and current_steps:
            placeholders_used = [ph.strip('<>').strip('"') for step in current_steps for ph in self.placeholder_pattern.findall(step) if ph.endswith('>')]
            for ph in placeholders_used:
                if ph not in examples_headers:
                    step_index = next((idx for idx, step in enumerate(current_steps) if f"<\"{ph}\">" in step), -1)
                    report.add_issue(
                        "Placeholder Mismatch",
                        f"Placeholder '<\"{ph}\">' does not match any column heading in the Examples table: {examples_headers}",
                        step_index + scenario_start_line + 1 if step_index != -1 else i
                    )
            # Check placeholder order
            header_order = {h: idx for idx, h in enumerate(examples_headers)}
            last_pos = -1
            for ph in placeholders_used:
                current_pos = header_order.get(ph, -1)
                if current_pos == -1:
                    continue
        ...
        logging.debug(f"Processed file {file} with {report.get_total_errors()} errors")
        return True
...
class GherkinCheckerApp:
    ...
    def __init__(self):
        ...
        self.checker = GherkinChecker(
            ...
            spell_cache=self.spell_cache,
            custom_words=self.custom_words,
            spell_lock=self.spell_lock,
            feature_type=self.feature_type,
            spellcheck_enabled=self.spellcheck_enabled,
            spellcheck_language=self.spellcheck_language
        )
    ...
    def open_report_checks_dialog(self):
        ...
        checks = [
            ...
            ("Special Character Errors", "special_chars"),
            ("Spacing Issues", "spacing"),
            ("Case Sensitivity Errors", "case_sensitivity"),
    ...
    def select_files(self):
        ...
        if files:
            ...
            logging.debug(f"Selected files: {files}")
    ...
    def select_folder(self):
        ...
        if folder:
            ...
            for file in feature_files:
                self.listbox_files.insert("end", f"{file}\n")
            logging.debug(f"Selected folder: {folder} with files: {feature_files}")
    ...
    def update_feature_type(self, feature_type):
        ...
        logging.debug(f"Updated feature type to: {self.feature_type}")
        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(xray_url, headers=headers, timeout=30) as response:
                    ...
                    tests = [{"key": key} for key in set(test_keys)]
                    logging.info(f"Found {len(tests)} tests in test plan {test_plan_key}: {test_keys}")
                    return tests

        except aiohttp.ClientError as e:
            logging.error(f"Network error fetching tests for test_plan {test_plan_key}: {e}")
            raise ValueError(f"Network error: {str(e)}")
        except Exception as e:
            logging.error(f"Unexpected error fetching tests for test_plan {test_plan_key}: {e}")
            raise ValueError(f"Unexpected error: {str(e)}")
    ...
    def run_checks(self, files, report_dir=None):
        ...
        if report_dir:
            for idx, (report, content) in enumerate(zip(reports, report_content)):
                ...
                try:
                    ...
                    logging.error(f"Failed to save report to {report_filename}: {e}")
                    self.root.after(0, lambda err=str(e): messagebox.showerror("Error", f"Failed to save report: {err}"))
        ...
        try:
            ...
            logging.info(f"Generated summary report: {summary_filename}")
            self.root.after(0, lambda: messagebox.showinfo("Success", f"Summary report generated: {summary_filename}\nTotal errors found: {total_errors}"))
        except IOError as e:
            logging.error(f"Failed to save summary report to {summary_filename}: {e}")
            self.root.after(0, lambda err=str(e): messagebox.showerror("Error", f"Failed to save summary report: {err}"))
    ...
    def run_checks_on_feature_files(self, issue_keys):
        try:
            ...
            self.root.after(0, lambda: self.status_indicator.configure(text="Checking", text_color="orange"))
            reports, summary_content = self.run_checks(feature_files, download_dir)
            if reports:
                self.root.after(0, lambda r=reports, s=summary_content: self.show_report_preview(r, s))
            self.root.after(0, lambda: self.status_indicator.configure(text="Idle", text_color="blue"))
            self.root.after(0, lambda: self.execute_button.configure(state="normal"))

        except Exception as e:
            logging.error(f"Error in run_checks_on_feature_files: {e}")
            self.root.after(0, lambda err=str(e): messagebox.showerror("Error", f"Failed to process feature files: {err}"))
            self.root.after(0, lambda: self.status_indicator.configure(text="Idle", text_color="blue"))
            self.root.after(0, lambda: self.execute_button.configure(state="normal"))
    ...
    def run_checks_on_test_plan(self, test_plan_key):
        try:
            ...
            self.root.after(0, lambda: self.status_indicator.configure(text="Checking", text_color="orange"))
            reports, summary_content = self.run_checks(feature_files)
            if reports:
                self.root.after(0, lambda r=reports, s=summary_content: self.show_report_preview(r, s))
            self.root.after(0, lambda: self.status_indicator.configure(text="Idle", text_color="blue"))
            self.root.after(0, lambda: self.execute_button.configure(state="normal"))
            ...
            logging.error(f"Error in run_checks_on_test_plan: {e}")
            self.root.after(0, lambda err=str(e): messagebox.showerror("Error", f"Failed to process test plan: {err}"))
            self.root.after(0, lambda: self.status_indicator.configure(text="Idle", text_color="blue"))
            self.root.after(0, lambda: self.execute_button.configure(state="normal"))

    def update_action_dropdown(self):
        if self.online_mode:
            self.action_dropdown.configure(values=["Run Checks on Files", "Download Feature Files", "Run Checks on Test Plan"])
        else:
    ...
    def execute_action(self):
        ...
        if action == "Run Checks on Files":
            files = self.listbox_files.get("1.0", "end").strip().split("\n")
            files = [f for f in files if f]
            if not files:
                messagebox.showwarning("Warning", "No files selected to check.")
                self.execute_button.configure(state="normal")
                return
            self.task_queue.put(("run_checks_offline", files))
        elif action == "Download Feature Files" and self.online_mode:
            issue_keys = self.entry_key.get().strip()
            if not issue_keys:
                messagebox.showwarning("Warning", "Please enter issue key(s) (e.g., PETM-1234).")
                self.execute_button.configure(state="normal")
                return
            self.task_queue.put(("download_feature_files", issue_keys))
        elif action == "Run Checks on Test Plan" and self.online_mode:
            test_plan_key = self.entry_key.get().strip()
            if not test_plan_key:
                messagebox.showwarning("Warning", "Please enter test plan key (e.g., TP-1234).")
                self.execute_button.configure(state="normal")
                return
            self.task_queue.put(("run_checks_on_test_plan", test_plan_key))
            ...
            try:
                if isinstance(task, tuple):
                    ...
                    if action == "run_checks_offline":
                        ...
                        self.run_checks_on_feature_files(files)
                    elif action == "download_feature_files":
                        self.run_checks_on_feature_files(args)
                    elif action == "run_checks_on_test_plan":
                        self.run_checks_on_test_plan(args)
                    elif action == "execute_action":
                        self.execute_action()
                self.task_queue.task_done()
            except Exception as e:
                logging.error(f"Error processing task {task}: {e}")
                self.root.after(0, lambda err=str(e): messagebox.showerror("Error", f"Task failed: {err}"))
                self.root.after(0, lambda: self.execute_button.configure(state="normal"))
